...
export default function UserTasks() {
  ...
  return (
    <div className="min-h-screen bg-ocean-900 p-4 md:p-8 text-white font-sans overflow-x-hidden">
      <div className="max-w-4xl mx-auto flex flex-col gap-6">
        
        <header className="flex justify-between items-center bg-ocean-800 p-6 rounded-[2rem] border border-white/10 shadow-2xl mt-4">
           <div className="flex items-center gap-4">
              <button 
                 onClick={() => navigate('/user/dashboard')}
                 className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:text-white transition shadow-inner"
              >
                  ←
              </button>
              <div>
                 ...
                 <p className="text-xs text-gray-400 font-medium mt-1">SUTRA Government Issued Priorities</p>
              </div>
           </div>
        </header>
        ...
        <div className="grid grid-cols-1 gap-6 pb-20">
            {activeTasks.length === 0 && (
                <div className="text-center py-20 bg-ocean-800/50 rounded-[2rem] border border-white/5">
                    <CheckCircle2 size={64} className="mx-auto text-green-500/30 mb-4" />
                    <h2 className="text-2xl font-black text-gray-500">Zone Clear!</h2>
                    <p className="text-gray-400 text-sm mt-2">No active approved tasks currently.</p>
                </div>
            )}
            ...
            {activeTasks.map((task, i) => (
                <motion.div 
                   ...
                   className="bg-ocean-800 p-6 rounded-[2rem] border border-red-500/20 shadow-[0_10px_30px_rgba(239,68,68,0.1)] flex flex-col gap-4 relative overflow-hidden"
                >
                    ...
                    <div className="flex justify-between items-start z-10">
                        <div className="flex items-center gap-3 mb-2">
                           <span className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full flex items-center gap-1 ${
                              task.priority === 'Critical' ? 'bg-red-500 text-white animate-pulse' :
                    ...
                    <div className="z-10">
                        ...
                        <div className="flex items-center gap-4 text-xs font-mono text-gray-400 mb-6 bg-ocean-900/50 p-3 rounded-xl border border-white/5">
                    ...
                    <div className="flex flex-col md:flex-row gap-3 z-10">
                        <button 
                           ...
                           className="flex-1 bg-[#1C2A40] border border-white/10 hover:bg-white/10 text-white py-4 rounded-xl font-bold flex justify-center items-center transition"
                        >
                           <Navigation size={18} className="mr-2"/> GPS Navigate
                        </button>
                        <button 
                           onClick={() => handleComplete(task.id)}
                           className="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 shadow-lg shadow-green-500/30 text-white py-4 rounded-xl font-black tracking-wide flex justify-center items-center gap-2 hover:scale-[1.02] transition-transform"
                        >
