...
export default function UserRewards() {
  ...
  return (
    <div className="min-h-screen bg-ocean-900 p-6 md:p-12 text-white font-sans relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto z-10 relative flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                   ...
                   <h2 className="text-6xl font-black text-white mt-4">{credits} <span className="text-2xl text-yellow-500">CR</span></h2>
                ...
                <motion.div 
                   whileHover={{ scale: 1.02 }}
                   className="bg-ocean-800 border border-white/10 p-8 rounded-[2rem] shadow-xl flex flex-col justify-center"
                >
                   ...
                   <h2 className="text-4xl font-black text-white mt-4 flex items-baseline gap-2">
                      {uploadedCount} <span className="text-xl text-gray-500 font-bold uppercase tracking-[0.2em]">Verified Uploads</span>
                   </h2>
                   ...
                   <div className="mt-6 flex flex-col gap-2">
                      ...
                      <div className="w-full h-3 bg-ocean-900 rounded-full overflow-hidden border border-white/5">
            ...
            {/* Leaderboard Section */}
            <div className="bg-ocean-800/50 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 shadow-2xl">
                <div className="flex items-center gap-3 mb-8">
                   ...
                   <div>
                      <h3 className="text-2xl font-black text-white tracking-tight">Global Leaderboard</h3>
                      <p className="text-sm font-medium text-gray-400">Top contributors in your region this week.</p>
                   </div>
                </div>

                <div className="flex flex-col gap-3">
                   {leaderboard.map((u, i) => (
                      <div key={i} className={`flex items-center justify-between p-4 rounded-2xl border ${u.name === 'Demo Citizen' ? 'bg-accent-blue/10 border-accent-blue/30' : 'bg-ocean-900/50 border-white/5'}`}>
                          <div className="flex items-center gap-4">
                             <span className={`w-8 h-8 flex items-center justify-center font-black rounded-full text-sm ${i === 0 ? 'bg-yellow-500 text-ocean-900 shadow-[0_0_15px_rgba(234,179,8,0.5)]' : i === 1 ? 'bg-gray-300 text-ocean-900' : i === 2 ? 'bg-orange-400 text-ocean-900' : 'bg-ocean-700 text-gray-400'}`}>
                                #{i + 1}
                             </span>
                          ...
                          <div className="font-black font-mono text-xl text-yellow-500 flex items-center">
                             {u.points} <span className="text-xs ml-1 text-gray-500">PTS</span>
                          </div>
                      </div>
                   ))}
                </div>
